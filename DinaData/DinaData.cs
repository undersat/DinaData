...
public class DinaData
{
    ...
    public DinaData(string json_current_values = "", string my_json_definition = "")
    {
        if (!caller_info())
        {
        ...
        if (string.IsNullOrEmpty(my_json_definition))
        {
            ...
            json_definition = ConfigurationManager.AppSettings[param_name];
    ...
    public DinaData setfield(string key, string value)
    {
    ...
    public String setfieldstr(string key, string value)
    {
        return setfield(key, value).ToString();
    }
